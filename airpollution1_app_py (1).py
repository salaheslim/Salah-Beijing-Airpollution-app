...
from sklearn.ensemble import RandomForestRegressor, AdaBoostRegressor, RandomForestClassifier # Added RandomForestClassifier
...
def load_and_preprocess_data_for_app(): # Renamed to be more descriptive
    try:
        ...
        # Removed on_bad_lines='skip' as your final CSV should be clean after preprocessing.
        df_app = pd.read_csv("analyzed_data.csv") # IMPORTANT: Reverted to 'analyzed_data.csv' as per our previous agreement for the final file name.

        # --- CRITICAL FIXES FOR DATA TYPES AFTER CSV LOAD ---
        # Ensure 'Date' is datetime (often saved as string) to avoid serialization errors and for plotting
        ...
        if 'year_month' in df_app.columns:
            try:
                # Attempt to convert to PeriodDtype for consistency with notebook (if relevant for app logic)
...
@st.cache_resource # Use cache_resource for models/scalers as they are objects
...
def modeling_and_prediction(data):
    ...
    if st.button("💾 Save Trained Model & Scaler"):
        ...
        with open(model_filename, 'rb') as f_model:
            st.download_button(f"Download {chosen_model_type} Model", f_model, file_name=model_filename, mime="application/octet-stream")
        if scaler is not None:
            with open(scaler_filename, 'rb') as f_scaler:
                st.download_button("Download StandardScaler", f_scaler, file_name=scaler_filename, mime="application/octet-stream")
...
    page = st.sidebar.radio("Go to", ["1. Data Overview", "2. EDA", "3. Modeling and Prediction"])

    if page == "1. Data Overview":
        data_overview(app_data)
    elif page == "2. EDA":
        eda(app_data)
    elif page == "3. Modeling and Prediction":
        modeling_and_prediction(app_data)
